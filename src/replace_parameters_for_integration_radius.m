function replace_parameters_for_integration_radius(integration_radius)
%%
...
while feof(fid_parameters) ~= 1 
    ...
    if isempty(strfind(line, 'b = ')) ~= 1
        line_num_integration_radius = ii;
    end
end
...
while feof(fid_parameters) ~= 1 
    ii = ii + 1;
    line = fgetl(fid_parameters);
    if ii == line_num_integration_radius
        line = integration_radius;
    end
