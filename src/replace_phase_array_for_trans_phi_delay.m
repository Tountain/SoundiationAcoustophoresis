function replace_phase_array_for_trans_phi_delay(phi_delay)
%%
% This function is used to change the information 'phi_delay' in the
...
while feof(fid_parameters) ~= 1 
    ...
    if isempty(strfind(line, 'phi_delay = ')) ~= 1
        line_num_trans_phi_delay = ii;
    end
end
...
while feof(fid_parameters) ~= 1 
    ii = ii + 1;
    line = fgetl(fid_parameters);
    if ii == line_num_trans_phi_delay
        line = phi_delay;
    end
